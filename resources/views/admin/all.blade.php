...
<section class="content-header">
  <h1>
    ...
    @if (count($alldata) > 0)
     <a href="{{ route('successTrxData') }}" class="btn btn-success float-right">Download Excell</a>
    @endif
  </h1>
</section>
    ...
    <div class="col-xs-12">
      <div class="box">
        <div class="box-header">
          <h3 class="box-title">ALL online payment + Manual success payment</h3>
        </div>
        <!-- /.box-header -->
        <div class="box-body">
          ...
          <table id="example1" class="table table-bordered table-striped">
            ...
            @foreach ($alldata as $single_data)
                ...
                <td>{{ $single_data->order_id }}</td>
	              ...
	              <td>{{ $single_data->mobile_1 }}<br>{{ $single_data->mobile_2 }}</td>
	              <td>
                  {{ $single_data->total_amount }}({{ $single_data->payment_method }})
                </td>
	              <td>{{ $single_data->created_at  }}</td>
	              <td>
	              	<a href="{{ route('reg.edit',$single_data->id) }}" class="btn btn-primary"><i class="fa fa-pencil"></i></a>
	              	<a href="#" data-id='{{ $single_data->id }}' data-type='main' class="btn btn-primary modal_switch" data-toggle="modal" data-target="#modal-default"><i class="fa fa-eye"></i></a>
	              </td>
	            </tr>
            @endforeach	
...
<script src="{{ asset('admin/bs_dt_btn/dt.btn.min.js') }}"></script>
<script src="{{ asset('admin/bs_dt_btn/btn.bs.min.js') }}"></script>
<script src="{{ asset('admin/bs_dt_btn/jszip.min.js') }}"></script>
<script src="{{ asset('admin/bs_dt_btn/pdfmake.min.js') }}"></script>
<script src="{{ asset('admin/bs_dt_btn/vsf_font.min.js') }}"></script>
<script src="{{ asset('admin/bs_dt_btn/btn.html5.min.js') }}"></script>
<script src="{{ asset('admin/bs_dt_btn/btn.print.min.js') }}"></script>
<script>
  $(function () {
